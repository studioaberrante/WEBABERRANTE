...
/* ---- FONDO DE ONDA DEL SHOWCASE ----
   Mismo gradiente animado que el CTA de Aberrante TV (tv.js),
   adaptado: el canvas es sticky y mide un viewport, así el costo
   de render no depende del alto de la sección. */
(function initShowcaseWave() {
  const canvas = document.getElementById('showcaseWave');
  ...
  function render() {
    ...
    for (let y = 0; y < height; y++) {
      for (let x = 0; x < width; x++) {
        ...
        const intensity = 0.45 + 0.5 * wave;
        const baseVal = 0.16 + 0.2 * fCos(ux + uy + time * 0.3);
        const blue = 0.3 * fSin(a * 1.5 + time * 0.2);
        const purple = 0.22 * fCos(d * 2 + time * 0.1);
  ...
  // Anima solo cuando la sección está en pantalla (ahorra CPU)
